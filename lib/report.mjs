...
const costBasisLabels = Object.freeze({
  equipment: "Solo apparecchiature",
  supply_install: "Fornitura e posa / CME",
  reproduction: "Costo di riproduzione chiavi in mano",
  other: "Altra base documentata",
});
...
export function buildMarkdownReport(data) {
  ...
  const lines = [
    ...
    "",
    data.works?.length
      ? `Sono inoltre presenti opere edilizie o qualitative: ${data.works.join(", ")}.`
      : "L’inventario non segnala opere edilizie o qualitative ulteriori rispetto alle dotazioni impiantistiche.",
    `**Inventario completo confermato dal compilatore:** ${data.inventoryConfirmed ? "Sì" : "No"}`,
    `**Metodo raccomandato dall’instradamento:** ${data.recommendedPath === "comparative" ? "Comparazione complessiva" : "Solo ampliamento impiantistico"}`,
    ...(data.overrideReason ? [`**Deroga professionale motivata:** ${data.overrideReason}`] : []),
    "",
    ...
    lines.push(
      ...
      ...data.plants.flatMap((plant, index) => {
        ...
        return [
          ...
          `- Base economica dichiarata: ${costBasisLabels[plant.costBasis] || missing(`base economica impianto ${index + 1}`)}`,
          `- Fattore di rialzo al costo di riproduzione: ${plant.applyReproductionUplift ? `${number(row.upliftFactor)} — ${field(plant.upliftSource, `fonte del rialzo impianto ${index + 1}`)}` : "Non applicato"}`,
          `- Valore normalizzato usato nel calcolo: ${money(row.normalizedNewValue)}`,
          ...
          ...(row.issues?.map(({ severity, message }) => `- ${severity === "invalid" ? "Errore bloccante" : "Avvertenza"}: ${message}`) || []),
          "",
        ];
      }),
      `- Stato di validazione: ${calculation.status}`,
      `- Versione tabella coefficienti: ${calculation.coefficientVersion}`,
